...
from utils.gmm_utils import calculate_matmul, calculate_matmul_n_times
...
class GaussianMixture(torch.nn.Module):
    ...
    def __init__(self, n_components, n_features, device,
                 covariance_type="full", eps=1.e-1, init_params="kmeans", mu_init=None,
                 var_init=None):
        """
    ...
    def _init_params(self):
        ...
        if self.covariance_type == "diag":
            ...
            if self.var_init is not None:
                ...
                self.var.data = self.var.data * 100
    ...
    def fit(self, x, delta=1e-3, n_iter=100, warm_start=False):
        ...
        if not warm_start and not self.params_fitted:
            self._init_params()
    ...
    def predict(self, x, probs=False):
        ...
        weighted_log_prob = self.centralize(weighted_log_prob)

        if probs:
            # print(weighted_log_prob.shape)
            prob = torch.softmax(weighted_log_prob.squeeze(dim=2), dim=1)
            # zero_idx = (p_k.sum((1, 2)) == 0)
            # if zero_idx.any():
            #     print('zero prob encountered')
            # p_k[zero_idx] = torch.ones(int(zero_idx.sum()), self.n_components).to(self.device).unsqueeze(-1)
            # inf_idx = (torch.isinf(p_k))
            # if inf_idx.any():
            #     print('inf prob encountered')
            #
            # p_k[inf_idx] = 1
            # prob = torch.squeeze(p_k / (p_k.sum(1, keepdim=True)), dim=2)
            assert not torch.isnan(prob).any()
            assert not (prob.sum(dim=1) == 0).any()
            return prob
        else:
    ...
    def m_step_with_response(self, x, resp):
        ...
        returns:
            ...
            var:        torch.Tensor (1, k, d)
        """
        assert not torch.isnan(resp).any()
    ...
    def update_parameter(self, mu, var, _pi=None, unseen=False):
        if _pi is not None:
            self.__update_pi(_pi)
        if unseen:
            return
        self.__update_mu(mu)
        self.__update_var(var)
    ...
    def calc_weighted_log_prob(self, x):
    ...
    def __update_var(self, var):
        ...
        if self.covariance_type == "full":
            assert var.size() in [(self.n_components, self.n_features, self.n_features), (
                ...
                self.n_features)], "Input var does not have required tensor dimensions (%i, %i, %i) or (1, %i, %i, %i), but instead {}".format(
                self.n_components, self.n_features, self.n_features, self.n_components, self.n_features,
                self.n_features, var.size())
    ...
    def adjust_var(self, sigma=1e-2):
        PD = torch.diag(sigma * torch.ones(self.var.size()[2]))
        self.var.data = self.var.data + PD.repeat(self.var.size()[0], 1, 1).to(self.device)
    ...
    def get_kmeans_mu(self, x, n_centers, init_times=50, min_delta=1e-3):
        ...
        for i in range(init_times):
            tmp_center = x[np.random.choice(np.arange(x.shape[0]), size=n_centers, replace=True), ...]
