...
# def split_and_reform_dataset(dataset, n_classes, n_clients, n_clusters, alpha, frac, rotation_ratio=0.5, seed=1234):
#     if n_clusters == -1:
...
#
#     rotations = np.random.binomial(1, rotation_ratio, len(dataset))
...
#         _, label = dataset[idx]
#         if_rotation = rotations[idx]
#         group_id = if_rotation
...
#
#     return clients_indices, rotation_idx

def split_and_reform_dataset(dataset, n_classes, n_clients, n_clusters, alpha, frac, seed=1234):
    if n_clusters == -1:
        n_clusters = n_classes
    ...
    rotations = np.random.binomial(1, 0.5, len(dataset))
    client_permute = np.random.binomial(1, 0.5, len(dataset))
    label_projection = np.random.permutation(n_classes)
    print(len(rotations))
    # all_labels = list(range(n_classes))
    # rng.shuffle(all_labels)
    # clusters_labels = iid_divide(all_labels, n_clusters)
    ...
    for cluster_id in range(n_clusters):
        ...
        for client_id, indices in enumerate(cluster_split):
            clients_indices[client_id] += indices
    permuted_clients_indices = []
    size = 0
    for step, client_idx in enumerate(clients_indices):
        if client_permute[step] == 1:

            permuted_clients_indices = permuted_clients_indices + client_idx
        size += len(client_idx)
    rotation_idx = np.where(rotations)
    return clients_indices, rotation_idx, permuted_clients_indices, label_projection
