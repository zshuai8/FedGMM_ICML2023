...
def main():
    ...
    for idx, file_name in enumerate(tqdm(file_names_list)):
        ...
        # rotation_idx_inv = np.setdiff1d(np.arange(data.shape[0]), rotation_idx)
        inverted_images = 1 - data
        # flipped_images = torch.flip(inverted_images, [2])
        rotated_images = torch.rot90(inverted_images, 1, [1, 2])
        rotated_images = torch.rot90(rotated_images, 1, [1, 2])
        rotated_images = torch.rot90(rotated_images, 1, [1, 2])
